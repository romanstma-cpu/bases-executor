// sunc.h - the full Solara UNC function table. Every entry is bound to a real
// rbx:: operation via the scanner in rbx.cpp. No fake placeholders: if a symbol
// can't resolve, the function returns an error string naming the missing symbol
// so the harness reports it instead of silently passing.
#pragma once
#include <cstdint>
...
namespace sunc {
    ...
    // Registers all UNC globals + debug.* into the given Lua state.
    // Returns the count of functions successfully bound (0..N). The harness
    // compares this to the known total to compute sUNC %.
    int  open(lua_State* L);

    // Individual bindings (declared so harness can self-verify each by name).
    ...
    int  l_request(lua_State* L);          // syn.request
